...
from ACO.Coordinate import Coordinate
...
class TSPData:
    ...
    def build_distance_lists(self):
        ...
        for i in range(number_of_products):
            ...
            for j in range(number_of_products):
                self.distances[i].append(self.product_to_product[i][j][0].size())
            self.start_distances.append(self.start_to_product[i][0].size())
            self.end_distances.append(self.product_to_end[i][0].size())
        return
    ...
    def write_action_file(self, product_order, file_path):
        ...
        string += ";\n"
        string += str(self.start_to_product[product_order[0]][0])
        string += "take product #"
        ...
        for i in range(len(product_order) - 1):
            frm = product_order[i]
            to = product_order[i + 1]
            string += str(self.product_to_product[frm][to][0])
            string += "take product #"
            string += str(to + 1)
            string += ";\n"
        string += str(self.product_to_end[product_order[len(product_order) - 1]][0])
